...
void AD9854_Init(void)
{
    ...
    AD9854_RST=0;
    /*
	Control DAC PD:开关振幅控制输出，专门针对PIN52输出的预先信号做振幅调制，=1关
	DAC PD 开关DAC转换 =1关
	MODE0~2	工作模式 0:SingleTone单调模式 1:Unramped FSK 无过渡频移键控模式 PIN29为低选择F1，为高选择F2，从F1到F2的变化是瞬时的
	2:倾斜频移键控再工作开始之前，控制寄存器中的CLR ACC1位必须清零，保证频率累加器从全0输出开始
	OSK EN 设置位高电平数字倍增器使能。否则，如果OSK EN设置为低，数字倍增器负责控制振幅将会
	被旁路，I和Q的输出将会被设置为满振幅
	OSK INT 高电平选择的是输出幅度渐变上升或者下降的线性内部控制。OSK INT是低电平时转换为用户
	可编程12位寄存器的数字倍增器控制，允许用户以任何方式改变振幅转换
	Bypass Inv Sinc被设置位高，反正弦滤波器会被旁路，供电就会被保持(就是没有供电，可以降低工号，减少发热)
	当不需要时DAC的非PD位设置为高DAC就会被关闭，control DAC也是一样，将control DAC PD置高将关闭DAC
	*/

	AD9854_WR_Byte(0x1d,0x00);	           //关闭比较器
	AD9854_WR_Byte(0x1e,CLK_Set);	   //设置系统时钟倍频，这个寄存器的下五位都是用来输入分频的，所以0~31            
	AD9854_WR_Byte(0x1f,0x00);	   //设置系统为模式0，由外部更新
	AD9854_WR_Byte(0x20,0x60);	   //01100000 设置为可调节幅度，取消插值补偿 
